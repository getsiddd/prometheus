...
# ===============================
# ATTACH DETECTED SHARED MEMORY
# ===============================
def attach_detected_memory(cam_id):
    name = f"detected-{cam_id}"
    try:
        shm = shared_memory.SharedMemory(name=name)
        print(f"[Detected Viewer] Attached to {name}")
        return shm
...
def main():
    ...
    print("[Detected Viewer] Starting...")

    # ✅ Make window resizable
    cv2.namedWindow("Detected Camera Viewer", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Detected Camera Viewer", 1280, 720)
    ...
    while True:
        ...
        for cam in cameras:
            ...
            if shms[cam_id] is None:
                shms[cam_id] = attach_detected_memory(cam_id)
        ...
        if grid is not None:
            cv2.imshow("Detected Camera Viewer", grid)
    ...
    print("[Detected Viewer] Closed cleanly.")
