...
export async function POST(req) {
  try {
    ...
    const autoMatch = Boolean(body?.autoMatch);
    const matchOptions = body?.matchOptions || {};

    if (!Array.isArray(cameras) || cameras.length < 2) {
      return NextResponse.json({ error: "At least 2 calibrated cameras are required" }, { status: 400 });
    }
    if (markers.length < 1 && !autoMatch) {
      return NextResponse.json({ error: "At least 1 marker is required unless autoMatch is enabled" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: err instanceof Error ? err.message : "Triangulation failed" }, { status: 500 });
  }
}
